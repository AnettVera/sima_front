...
const DashboardUser = () => {
  ...
  const availableArticles =
    storage && allArticles
      ? allArticles.filter(
          (a) => a.category?.id === storage.category?.id && a.quantity > 0
        )
      : [];
  const handleAddArticle = async () => {
    if (!selectedArticleId || !addQuantity) return;
    const result = await assignArticleToStorage(
      storage.id,
      selectedArticleId,
      addQuantity
    );
  ...
  const handleMoveArticle = async (articleId, articleName) => {
    ...
    if (quantity) {
      try {
        await articleService.removeFromStorage(
          storage.id,
          articleId,
  ...
  const groupedArticles =
    storage?.articles?.reduce((acc, article) => {
      if (!acc[article.id]) {
        acc[article.id] = {
          ...article,
          quantity: 1,
  ...
  const totalQuantity = uniqueArticles.reduce(
    ...
    return (
      <div className="min-h-screen flex  justify-center bg-background ">
        <div className="text-center">
          <h1 className="text-3xl font-bold text-titleText mb-4">
            Sin almacén asignado
          </h1>
          <p className="text-text">
            Por el momento no tienes ningún almacén asignado. Contacta con el
            administrador.
          </p>
        </div>
      </div>
  ...
  return (
    <div className="space-y-6">
      {/* Título del almacén con categoría */}
      <h2 className="text-xl font-semibold text-text-primary mb-4">
        Almacén: {storage.storageIdentifier} - Categoría:{" "}
        {storage.category.categoryName}
      </h2>

      {/* Cards */}
      <div
        className={`grid grid-cols-1 ${
          isStorageActive ? "md:grid-cols-5" : "md:grid-cols-4"
        } gap-6 mb-8`}
      >
        {/* Productos */}
        ...
        {/* Registrar producto → solo si está activo */}
        {isStorageActive && (
          <div
            className="card text-center cursor-pointer"
            onClick={() =>
              navigate(`/articles/user/`, {
                state: { category: storage.category?.categoryName },
              })
            }
          >
            <button>
              ...
              {uniqueArticles.map((article) => (
                ...
                >
                  ...
                  <td className="py-3 px-4">
                    <button
                      onClick={() =>
                        handleMoveArticle(article.id, article.articleName)
                      }
